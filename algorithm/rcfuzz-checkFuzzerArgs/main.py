...
class Schedule_Base(SchedulingAlgorithm):
    ...
    def execution_cpu_assign(self,  new_cpu_assign, execution_time: int) -> bool:
        '''
        return whether we find new coverage during execution phase
        '''
        global OUTPUT, JOBS
        ...
        for fuzzer in run_fuzzers:
            ...
            while focusRemainTime > 0 :
                ...
                logger.info(f'main 501 - execution round : {focusRound}end result - fuzzer : {fuzzer}, previousBitmap : {previousBitmap}, currentBitmap : {currentBitmap}, previousBug : {previousBug}, currentBug : {currentBug}, focusSuccess : {focusSuccess}, focusFail : {focusFail}, fuzzer success :  {self.rcFuzzers[fuzzer].S}, fuzzer fail : {self.rcFuzzers[fuzzer].F}, fuzzer threshold : {self.rcFuzzers[fuzzer].threshold}, fuzzer branch difficulty : {self.rcFuzzers[fuzzer].diff}, focusRemainTime : {focusRemainTime}, focusRunTime : {focusRunTime}')
...
def main():
    ...
    while not is_end_global():
        logger.info(f'main 039 - sleep to wait final coverage')
        time.sleep(300)
