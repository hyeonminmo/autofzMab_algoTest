...
class Schedule_Autofz(Schedule_Base):
    ...
    def one_round(self):
        ...
        thresholdInit = self.diff_threshold_base/10

        if self.round_num == 1:
            if current_bitmap - preparation_bitmap > self.tsFuzzers[selected_fuzzers[0]].threshold or current_unique_bug - preparation_unique_bug > 0:
                #thompson.updateFuzzerCount(self.tsFuzzers, selected_fuzzers, 1)
                self.tsFuzzers[selected_fuzzers[0]].threshold += thresholdInit
            else:
                #thompson.updateFuzzerCount(self.tsFuzzers, selected_fuzzers, 0)
                self.tsFuzzers[selected_fuzzers[0]].threshold *= 0.5
        else:
            if current_bitmap - previous_bitmap > self.tsFuzzers[selected_fuzzers[0]].threshold or current_unique_bug - previous_unique_bug > 0:
                #thompson.updateFuzzerCount(self.tsFuzzers, selected_fuzzers, 1)
                self.tsFuzzers[selected_fuzzers[0]].threshold *= thresholdInit
            else:
                #thompson.updateFuzzerCount(self.tsFuzzers, selected_fuzzers, 0)
