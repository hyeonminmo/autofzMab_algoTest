...
#                return True
    ...
    def focus_cpu_assign(self,  new_cpu_assign, focus_time: int) -> bool:
        ...
        focusThresholdInit = self.diff_threshold_base/10
        ...
        for fuzzer in run_fuzzers:
            ...
            focusRemainTime = t
            logger.info(f'main 701 - focus remain time init : {focusRemainTime}, focusThresholdInit " {focusThresholdInit}')
            focusRound = 1
            ...
            while focusRemainTime > 0 :
                ...
                if currentBitmap - previousBitmap > self.tsFuzzers[fuzzer].threshold  or currentBug - previousBug >0:
                    ...
                    focusSuccess += 1
                    #focusRemainTime += t
                    self.tsFuzzers[fuzzer].threshold += focusThresholdInit
                else:
...
class Schedule_Autofz(Schedule_Base):
    ...
    def one_round(self):
        ...
        current_time = time.time()
        timeout_seconds = utils.time_to_seconds(ARGS.timeout)
        currentRunTime = current_time - START_TIME
        
        logger.info(f'main 1001 - round {self.round_num} end result - time : {currentRunTime}, selectedFuzzer : {selected_fuzzers}, previousSelectedFuzzer : {self.previousSelectedFuzzer} ')

        if currentRunTime <= (timeout_seconds*0.5):
            if self.previousSelectedFuzzer == selected_fuzzers[0] :
                if current_bitmap == previous_bitmap and current_unique_bug == previous_unique_bug:
                    self.tsFuzzers[selected_fuzzers[0]].straightFail += 1
                else:
                    self.tsFuzzers[selected_fuzzers[0]].straightFail = 0
            else:
                if self.previousSelectedFuzzer != '':
                    self.tsFuzzers[self.previousSelectedFuzzer].straightFail = 0
        self.previousSelectedFuzzer = selected_fuzzers[0]

        for fuzzer in FUZZERS:
            #self.tsFuzzers[fuzzer].stack += 1
            logger.info(f'main 031 - round {self.round_num} end result - fuzzer : { fuzzer }, fuzzer_success : { self.tsFuzzers[fuzzer].S }, fuzzer_fail : { self.tsFuzzers[fuzzer].F }, fuzzer run time " {self.tsFuzzers[fuzzer].total_runTime}, fuzzer stack : {self.tsFuzzers[fuzzer].stack}, fuzzer straightFail : {self.tsFuzzers[fuzzer].straightFail}')
