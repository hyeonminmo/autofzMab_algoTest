...
do
    mkdir -p /d/p/justafl/newTarget/$newTarget_target
    mkdir -p /d/p/aflasan/newTarget/$newTarget_target
done

{
    cd /autofz_bench/newTarget/e2fsprogs && ./configure --disable-shared &&\
        make clean && \
        make -j &&\
        cp e2fsck/e2fsck /d/p/justafl/newTarget/e2fsck &&\
        make clean && AFL_USE_ASAN=1 make -j &&\
        cp e2fsck/e2fsck /d/p/aflasan/newTarget/e2fsck &&\
        make clean
} &
