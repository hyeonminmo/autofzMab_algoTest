...
for newTarget_target in "${newTarget_targets[@]}";
do
    mkdir -p /d/p/aflclangfast/newTarget/$newTarget_target
    mkdir -p /d/p/aflclangfastcmplog/newTarget/$newTarget_target
done

{
    cd /autofz_bench/newTarget/e2fsprogs && ./configure --disable-shared &&\
        make clean && \
        make -j &&\
        cp e2fsck/e2fsck /d/p/aflclangfast/newTarget/e2fsck &&\
        make clean && AFL_LLVM_CMPLOG=1 make -j &&\
        cp e2fsck/e2fsck /d/p/aflclangfastcmplog/newTarget/e2fsck &&\
        make clean
} &
