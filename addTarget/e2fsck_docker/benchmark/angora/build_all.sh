...
do
    mkdir -p /d/p/angora/fast/newTarget/$newTarget_target
    mkdir -p /d/p/angora/taint/newTarget/$newTarget_target
done
...

{
    cd /autofz_bench/newTarget/e2fsprogs && ./configure --disable-shared &&\
        make clean && \
        make -j &&\
        cp e2fsck/e2fsck /d/p/angora/fast/newTarget/e2fsck &&\
        make clean && USE_TRACK=1 make -j &&\
        cp e2fsck/e2fsck /d/p/angora/taint/newTarget/e2fsck &&\
        make clean
} &
