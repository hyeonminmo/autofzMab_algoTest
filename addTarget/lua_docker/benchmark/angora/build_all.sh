...
do
    mkdir -p /d/p/angora/fast/magma/$magma_target
    mkdir -p /d/p/angora/taint/magma/$magma_target
done

{
     cd /autofz_bench/magma/lua && \
        make -j clean && make -j liblua.a && \
        cp liblua.a /d/p/angora/fast/magma/lua/liblua.a && \
        make -j lua && \
        cp lua /d/p/angora/fast/magma/lua/lua && \
        make -j clean && USE_TRACK=1 make -j liblua.a && \
        cp liblua.a /d/p/angora/taint/magma/lua/liblua.a && \
        USE_TRACK=1 make -j lua && \
        cp lua /d/p/angora/taint/magma/lua/lua && \
        make clean
} &
