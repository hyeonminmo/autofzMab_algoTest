...
class Schedule_Autofz(Schedule_Base):
    ...
    def one_round(self):
        ...
        logger.info(f'main 104 - confirm early exit condition : {has_winner}')
        prep_end_time = time.time()
        fuzzer_info = get_fuzzer_info(self.fuzzers)
        after_prep_fuzzer_info = fuzzer_info

        after_prep_global_bitmap = after_prep_fuzzer_info['global_bitmap'].count()
        ...
        logger.info(f'main 027 - dynamic_prep_time_round: {self.dynamic_prep_time_round}, dynamic_focus_time_round: {self.dynamic_focus_time_round}')
        ...
        for fuzzer in FUZZERS:
            ...
            logger.info(f'main 031 - round {self.round_num} end result - fuzzer : { fuzzer }, total run time: {self.tsFuzzers[fuzzer].total_runTime}, prep run time : {self.tsFuzzers[fuzzer].prep_runTime}, focus run time: {self.tsFuzzers[fuzzer].focus_runTime},round total run time: {total_roundRunTime}, after_prep_global_bitmap: {after_prep_global_bitmap}')
        ...
        #logger.info(f'main 031 - round {self.round_num} end result - previous_bitmap: {previous_bitmap}, current_bitmap: {current_bitmap}')

       # if current_bitmap - previous_bitmap > self.diff_threshold:
       ...
       # for fuzzer in FUZZERS:            
       #     logger.info(f'fuzzer : { fuzzer }, fuzzer_success : { self.tsFuzzers[fuzzer].S }, fuzzer_fail : { self.tsFuzzers[fuzzer].F }')
